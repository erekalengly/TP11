    ...
    <div class="sidebar">
      ...
      <div class="category" v-for="category in categories" :key="category">
        ...
        <ul>
          ...
          <li>Samsung</li>
          <li>LG</li> -->
          <li  v-for="subcategory in category.subcategories" :key="subcategory"><span @click="get_product_by_sub(subcategory._id)">{{subcategory.subcategory_name}}</span></li>
        </ul>
      </div>
    </div>
    <div class="items">
...
export default {
  ...
  methods: {
    async get_product_by_sub(sub_id){
  
      let response_products = await axios.get(
      "http://localhost:3000/api/subcategory/get_by_id/" + sub_id
    );
    this.products=[]
  ...
  async mounted() {
    ...
    let response_products = await axios.get(
      "http://localhost:3000/api/product/get_products"
    );
    ...
    let reponse_categories = await axios.get(
      "http://localhost:3000/api/category/get_categories"
    );
